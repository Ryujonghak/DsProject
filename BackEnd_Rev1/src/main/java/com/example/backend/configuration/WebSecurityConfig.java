...
import com.example.backend.security.auth.CustomOAuth2UserService;
import com.example.backend.security.auth.OAuth2SuccessHandler;
...
//            TODO: /api/**, auth-redirect 통과시키는(접속허용) 걸로 수정
        .authorizeRequests().antMatchers("/api/**", "/auth-redirect").permitAll()
            // 그 외 url 은 인증 필요
        .anyRequest().authenticated()
//            OAuth2 로그인 기능에 대한 진입점 추가 -> 실행될 클래스 정의
            .and()
//            간편 로그인 인증
            .oauth2Login()
...
////                리소스 서버(즉, 소셜 서비스들)에서 사용자 정보를 가져온 상태에서 추가로 진행하고자 하는 기능을 명시할 수 있음
            .userService(customOAuth2UserService); // 유저정보 db 저장
